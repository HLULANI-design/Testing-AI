...
const NewPrompt = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    // Only send the fields required by your backend
    const run = await createRun({ task, inputText, contextUrl });
    setSuccess(true);
    setInputText("");
    setContextUrl("");
    // Pass the backend response (run.output or run.res) to RunDetails
    setTimeout(() => navigate(`/runs/${run.id}`, { state: { response: run.output || run.res } }), 1200);
  };
